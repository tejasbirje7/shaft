...
public class IdentityDAOImpl implements IdentityDAO {
    ...
    @Autowired
    public IdentityDAOImpl(IdentityRepository identityRepository) throws Exception {
    ...
    @Override
    public Mono<Map<String, Integer>> checkIdentity(int account,Map<String,Object> details) {
            ...
            return identityRepository.checkIfIExistsForFp(fp)
                    ...
                    .map(fpDetails -> {
                        ...
                        if(fpDetails.isEmpty()) {
                            ...
                            // `i` exists return `fp`
                            ACCOUNT_ID.remove();
                            resp.put("i",fpDetails.get(0).getIdentity());
                        }
                        ACCOUNT_ID.remove();
                        return resp;
                    });
